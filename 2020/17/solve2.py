...
def cells_range(cells):
    ...
    for x,y,z,w in cells:
        ...
        rw = range(min(w, rw.start), max(w+1, rw.stop))
    return rx, ry, rz, rw
...
def cells_print(cells):
    rx, ry, rz, rw = cells_range(cells)
    for w in rw:
        for z in rz:
            print(f'z={z}, w={w}')
            for y in ry:
                print(''.join('#' if (x,y,z,w) in cells else '.' for x in rx))
            print('')
...
def step(cells):
    new_cells = set()
    rx, ry, rz, rw = expanded_cells_range(cells)
    for x in rx:
        for y in ry:
            for z in rz:
                for w in rw:
...
def solve2(input, cycles=6, verbose=False):
    cells = parse(input)
    if verbose:
        cells_print(cells)
...
#solve2(EXAMPLE, 2, True)
assert solve2(EXAMPLE) == 848
