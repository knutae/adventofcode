...
class InteractiveInput(Input):
    def __init__(self, pre_commands):
        self.buffer = [ord(c) for c in ('\n'.join(pre_commands) + '\n')]
        self.buffer.reverse()
    
    def next(self):
        if len(self.buffer) == 0:
            line = sys.stdin.readline()
            self.buffer = [ord(c) for c in line]
            self.buffer.reverse()
        assert len(self.buffer) > 0
        return self.buffer.pop()

def all_combinations(inventory):
    for length in range(1, len(inventory)):
        for combo in itertools.combinations(inventory, length):
            yield combo

def main():
    with open('input') as f:
        program = [int(x) for x in f.read().strip().split(',')]
    commands = list(PRE_COMMANDS)
    current_inventory = INVENTORY
    for combo in all_combinations(INVENTORY):
        for item in current_inventory:
            commands.append('drop ' + item)
        for item in combo:
            commands.append('take ' + item)
        commands.append('west')
        current_inventory = combo

    output_line = []
    for c in run_yield(program, InteractiveInput(commands)):
        if c == 10:
